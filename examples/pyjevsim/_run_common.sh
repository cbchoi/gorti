#!/usr/bin/env bash
# Sourced by rtid_run.sh, producer_run.sh, consumer_run.sh, verify_run.sh.
# Holds the defaults so all four scripts agree on which rtid to dial,
...
#   RTID_URL=grpc://127.0.0.1:9000 ./producer_run.sh
#
# Mirrors examples/pyjevsim-relay-cross-process/_run_common.sh
# deliberately -- the two examples share a mental model.
...
# Ports — match docs/rtid-tui.md conventions. Note these are the same
# defaults the relay-cross-process scripts use, so do NOT run the two
# examples concurrently against the default ports — pick one or
# override for the other.
...
# Per-federate tail ticks. Producer tail is always 0 (a pure source
# has no incoming work past its drain phase). Consumer tail is sized
# so a producer emit on its last drain tick is still received.
: "${CONSUMER_TAIL_TICKS:=40}"

# Result + log destinations.
: "${RESULT_DIR:=/tmp/pyjevsim-cross}"
mkdir -p "${RESULT_DIR}"
...
#
# Usage:
#   "${PYTHON}" producer_main.py ... || RC=$?
#   report_result "${RC:-0}" "${RESULT_PATH}" producer_run published
report_result() {
    local rc=$1
    local result_path=$2
    local script=$3
