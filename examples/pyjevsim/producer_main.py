...
Boot sequence:
  ...
  4. Run an untimed tick loop for ``ticks + drain_ticks`` cycles.
     Each cycle calls ``Producer.output_handler``, which stamps the
     next monotonic seq and emits it on the ``out_seq`` port; the
     loop translates that into ``send_interaction("ProducerOutput",
     {"_payload": <bytes>})``.
  5. Write {"published": [seq, ...]} to the result file and exit.
"""
...
async def run(
    *,
    ...
    async with RtiConnection.connect(url) as rti:
        async with rti.join_federation(spec, federate_name="producer") as fed:
            print(
