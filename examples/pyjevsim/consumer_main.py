...
async def run(
    *,
    ...
    async with (
        ...
        rti.join_federation(spec, federate_name="consumer") as fed,
    ):
            print(
            ...
            # A fixed tail is normally sufficient, but it can expire first
            # when the producer process is descheduled for several seconds.
            # Keep the subscription alive until the known finite workload is
            # complete, with a bounded wait so a genuine delivery failure is
            # still surfaced by the runner.
            completion_deadline = (
                asyncio.get_running_loop().time()
                + max(5.0, tail_ticks * tick_period)
            )
            while (
                len(model.received) < ticks
                and asyncio.get_running_loop().time() < completion_deadline
            ):
                await drain_externals_now(
                    fed,
                    model,
                    {"ProducerOutput": "in_seq"},
                )
                if len(model.received) < ticks:
