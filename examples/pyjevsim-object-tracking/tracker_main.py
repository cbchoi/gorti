...
async def run(
    *,
    ...
    model = VehicleTracker(tick_step)
    async with (
        ...
        print(
            f"tracker[{name}]: discovered {n_disc} Vehicle instance(s); entering NMRA loop",
            file=sys.stderr,
            flush=True,
        )
        ...
        for i in range(1, cycles + 1):
            ...
            if model.output_handler():
                raise RuntimeError("tracker model unexpectedly emitted output")
