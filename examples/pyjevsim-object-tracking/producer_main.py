...
class VehicleProducer:
    ...
    def time_advance(self) -> float:
        # The TimeService grant gates the cycle, not this value.
        return 1.0
    ...
    def output_handler(self) -> dict[str, Any]:
        # The producer's "output" is an attribute update, not a port.
        # The runner emits update_attributes directly using the grant
        # time as both timestamp + position.
        return {}
    ...
    async with RtiConnection.connect(url) as rti:
        async with rti.join_federation(spec, federate_name=name) as fed:
            ...
            obj_handle = await fed.register_object_instance(
                "Vehicle", instance_name=f"vehicle-{name}",
            )
            print(
            ...
            # Updating with timestamp=t requires t <= currentTime +
            # lookahead, which holds while tick_step <= lookahead.
            for i in range(1, cycles + 1):
                t = i * tick_step
                pos = model.position_at(t)
                spd = model.speed_at(t)
                ...
                model.published.append({
                    "t": granted, "position": pos, "speed": spd,
                })
