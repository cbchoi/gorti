...
if ! compgen -G "${REPO_ROOT}/pysdk/rti1516e/_generated/rti/v1/*_pb2.py" >/dev/null; then
    echo "ieee1516e-typed-smoke: generating gRPC bindings" >&2
...
echo "ieee1516e-typed-smoke: running live typed-handle verification" >&2
cd "${REPO_ROOT}"
"${PYTHON_BIN}" -m pytest -q -s \
    pysdk/tests/spec/m28/test_ieee1516e_typed_smoke.py::test_spec_m28_ieee1516e_typed_smoke
