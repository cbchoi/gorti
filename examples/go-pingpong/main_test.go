...
func buildRtidOnce(t *testing.T) string {
	...
	rtidBinOnce.Do(func() {
		...
		// A newly written executable can spend longer than the demo timeout in
		// Windows security scanning before its first instruction runs. Preflight
		// the exact binary outside the demo timer so the smoke and runtime-budget
		// tests measure rtid rather than first-launch scanning.
		if runtime.GOOS == "windows" {
			preflight := exec.Command(bin, "-version") //nolint:gosec // test-built binary
			preflight.Stderr = os.Stderr
			preflight.Stdout = os.Stdout
			if err := preflight.Run(); err != nil {
				rtidBinErr = fmt.Errorf("preflight rtid binary: %w", err)
				return
			}
		}
		rtidBinPath = bin
