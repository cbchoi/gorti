...
FEDERATION_NAME = "pyjevsim-dashboard-bridged"
...
def common_parser(prog: str) -> argparse.ArgumentParser:
    ...
    p.add_argument("--publisher-done-file", default=None)
    return p
...
def mark_ready(path: str | None) -> None:
    if path is None:
        return
    ready = Path(path)
...
def mark_complete(path: str | None) -> None:
    mark_ready(path)
