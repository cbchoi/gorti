...
async def _rendezvous(
    ...
    await wait_for_announced(
        fed, label, timeout=timeout,
        on_tick=received_during_sync.append,
    )
    ...
    *,
    ...
    async with RtiConnection.connect(url) as rti:
        async with rti.join_federation(spec, federate_name=name) as fed:
            ...
            await _rendezvous(fed, START_LABEL, rendezvous_timeout, received_during_sync)
            achieved.append(START_LABEL)
            synchronized.append(START_LABEL)
            ...
            await _rendezvous(fed, END_LABEL, rendezvous_timeout, received_during_sync)
            achieved.append(END_LABEL)
            synchronized.append(END_LABEL)
