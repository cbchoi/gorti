"""Pitch-typed smoke federate — M28 W4.

A federate written using ONLY the Pitch-style Rti1516eAmbassador
methods AND the M28 Pitch-style typed handles + typed collections.
...
This is the sibling of ``examples/pitch-shape-smoke/smoke_federate.py``
that proves Pitch-port parity at the TYPE level: a federate ported
from a commercial RTI (Pitch HLA Evolved, Portico, MAK RTI) compiles
...
in the public ``rti1516e`` namespace with the exact Pitch names.
...
Runnable standalone via:
    ...
    python examples/pitch-typed-smoke/smoke_federate.py grpc://127.0.0.1:8080
"""
...
FEDERATION_NAME = "pitch-typed-smoke"


class TypedSmokeFederate(Rti1516eAmbassador):
    """Pitch-typed federate — uses only typed handles + typed collections."""
...
def run_publisher(
    ...
    *,
    ...
    try:
        ...
        # 2. Publish using a typed ``AttributeHandleSet`` (Pitch idiom).
