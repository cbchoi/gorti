...
What's *different* from the in-process test:
  ...
  - Drop counts vary across runs because the untimed relay processes
    are paced by independent event loops. The invariants still hold,
    so the parametrised tests assert conservation without pinning
...
# the other example tests use after the cross-collection housekeeping
# fix.
import importlib.util as _il  # noqa: E402

_spec = _il.spec_from_file_location(
    "_pyjevsim_relay_cross_process_runner", _HERE / "runner.py"
)
_runner = _il.module_from_spec(_spec)
sys.modules.setdefault("_pyjevsim_relay_cross_process_runner", _runner)
_spec.loader.exec_module(_runner)
run_once, verify = _runner.run_once, _runner.verify


def _skip_without_rtid() -> None:
    """Skip when neither a native daemon nor a Go builder is available."""
    if not _runner._default_rtid().is_file() and shutil.which("go") is None:
        pytest.skip(
            "rtid is unavailable and Go is not on PATH"
        )
...
def test_relay_cross_process_default_config() -> None:
    ...
    """
    _skip_without_rtid()
    result = asyncio.run(run_once(federate_timeout=60.0, keep_workdir=False))
