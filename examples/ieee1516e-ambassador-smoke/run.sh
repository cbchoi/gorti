...
fail() {
    echo "ieee1516e-ambassador-smoke: $*" >&2
    exit 1
}
...
if ! compgen -G "${REPO_ROOT}/pysdk/rti1516e/_generated/rti/v1/*_pb2.py" >/dev/null; then
    echo "ieee1516e-ambassador-smoke: generating gRPC bindings" >&2
...
echo "ieee1516e-ambassador-smoke: running live publisher/subscriber verification" >&2
cd "${REPO_ROOT}"
"${PYTHON_BIN}" -m pytest -q -s \
    pysdk/tests/spec/m27/test_ieee1516e_ambassador_smoke_cross.py::test_spec_m27d_ieee1516e_ambassador_cross_federate
