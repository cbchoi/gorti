#!/usr/bin/env bash
# ci-gates.sh runs the full DLC conformance gate sequence.
#
...
# Each gate protects a generated interface or executable conformance boundary:
#   - stubs: generated bindings must match the protocol definitions.
#   - cpp: every conformance fixture target must be built explicitly.
#   - sweep: verdicts are checked in both directions.
set -u -o pipefail
...
gate_cpp() {
  ...
  # conf_* fixture targets are EXCLUDE_FROM_ALL and must be built explicitly.
  # Anchor on the help line format ("... <target>") so substrings of
  # test_conf_* driver targets do not produce phantom target names.
  local conf_targets
...
gate_static() {
  ...
  bash scripts/check-no-debug-prints.sh 2>/dev/null || true
}
    ...
    case "$expect" in
      ...
      FULL|PARTIAL)
        ...
        case "$expect:$rc" in
          ...
          PARTIAL:0) fail "$fixture: now FULL but expected PARTIAL; update EXPECTED_VERDICTS.txt" ;;
...
gate_ivct() {
  ...
  [ -x bin/rtid ] || die "bin/rtid missing (run the go stage first)"
  # Regenerate pysdk/rti1516e/_generated after protocol changes.
  "${PYTHON:-python}" -m pytest tests/conformance/rti/ivct-subset -v \
    || fail "ivct subset failed; verify Python dependencies and generated bindings"
}
