...
| **M33** | Agents I/J/K/L/M/N/O + orchestrator | DLC callbacks + exceptions + ownership + DDM + obj-mgmt (~100/200) | 40 lockfile TUs at PASS (unchanged from M32 — I/N/M discovered E's stubs were more spec-complete than the M33 plan assumed; work shifted to test generation + gap-fixes). 121 Annex C exception classes runtime-tested via new gorti-only test at `cppsdk/tests/dlc/conformance/_runtime/test_exceptions_throw_and_catch.cpp` (5 gtest cases, all PASS — first real GREEN conformance test in the DLC track). FederateAmbassador all 60 callbacks decorated with `RTI_THROW(FederateInternalError)` (row 4.37 closed). `cppsdk/src/dlc/RTIambassadorImpl.cpp` §6 (13 methods), §7 (11 methods), §9 (deleteRegion const&), §10 (48 methods including advisory switches + evokeCallback single-arg + decode\*Handle × 9) sections replaced with real impls (M17 wire-up deferred, methods throw spec-legal exceptions with mandatory tag capture). 19 per-class encoding headers landed (`HLAfloat64BE.h` etc, resolves M32 PARSE\_FAIL bottleneck). 7 previously-parse-failing fixtures now parse; 2 `own_*` fixture bugs fixed (spec-invalid `attributeOwnershipDivestitureNotification` → `requestDivestitureConfirmation`; 3-arg `attributeOwnershipAcquisitionIfAvailable` → 2-arg per spec §7.9); 7 fixture missing-include bugs fixed. `librti1516e_dlc.a` builds clean (1052062 bytes) after post-merge fixup adding 19 `set()` bodies to `BasicDataElements.cpp` (Agent F added spec-mandated `set()` to header without impl). **First gorti↔Pitch parity diff on `om_helloworld_pubsub`: NOT_YET_ATTEMPTED** — fixture binaries build clean, but `amb->connect()` still throws "M32 stub" at runtime because §4 Federation Management (lines 32-120 of RTIambassadorImpl.cpp) was **not assigned to any M33 agent** (orchestrator dispatch gap; §4 is M34 pull-forward candidate). **DONE 2026-07-02** (with §4 acknowledged as M34-early scope). |
| **M34** | Agents AA/AB/AC/AD/AE/AF/AG | DLC §4 pull-forward + §5 Declaration + §8 Time Mgmt + encoding refinements + callback bridge (~40/200 additional GREEN) | Two-thread scope closed: (1) §4/§5/§8 pull-forward — AA replaces the 25 `m32_stub()` sites in `RTIambassadorImpl.cpp` §4 Federation Management with real impls wiring through to M17 transport (createFederationExecution / joinFederationExecution / resignFederationExecution / connect / disconnect + save/restore lifecycle); AC lands §5 Declaration Management wstring↔M17 shim (10 methods, publish/subscribe object+interaction classes); AB lands §8 Time Management shim (13 catalogue rows §9.1-§9.14 — enable/disable regulating+constrained, TAR/TARA/NER/NMRA/FQR, queries, advance grant dispatch). (2) Original M34 scope — AF lands §14 encoding refinements (composites + round-trip test, 36 gtests total for encoding surface); AE fills `HLAfloat64Time` / `HLAinteger64Time` full behavioral impl. AD lands `cppsdk/src/dlc/FederateAmbassadorBridge.{h,cpp}` — the callback dispatch bridge from gorti M17 event stream to DLC `FederateAmbassador` interface (unblocks subscriber-leg discover/reflect/receive/remove callbacks, 22 gtests). Also fixed during integration: `rti/pkg/fom/parser/strict.go` accepts the 4 spec-mandated advisory switches per IEEE 1516.2-2010 §F.10 (attributeScope/attributeRelevance/objectClassRelevance/interactionRelevance) — gorti M17 real HLA compliance gap; and 19 missing `BasicDataElements.cpp::set()` bodies unblocked fixture linking. AG runs the acceptance gate + first real gorti↔Pitch parity diff. **First gorti↔Pitch parity diff on `om_helloworld_pubsub`: PARTIAL** (`M34-PARITY-OUTCOME: PARTIAL 2/9 pub + 2/7 sub`) — publisher matches Pitch byte-identical on CONNECT + JOIN (2/9 events); CREATE is a fixture emission gap; PUBLISH / REGISTER / UPDATE / SEND / RESIGN pending §5+§6 impl remaining shim-throws. Subscriber matches on CONNECT + JOIN (2/7); SUBSCRIBE / DISCOVER / RECEIVE / REFLECT / RESIGN pending §5+§6 impl + callback bridge fanout. First checked-in evidence of the strict DLC surface producing spec-canonical output (`gorti-captured.*.log` committed alongside Pitch goldens). **DONE 2026-07-02** — see M34-{AA,AB,AC,AD,AE,AF,AG} merges. |
| **M35** | Agents BA/BB/BC/BD/BE/BF/BG | DLC §5+§6 §16 MOM real impls + `[[deprecated]]` on M17 rti1516e/*.h + `docs/MIGRATION_M17_TO_DLC.md` publish + IVCT-derived conformance subset + acceptance gate (~10/200 remaining GREEN + conformance gate) | Drives the M34 PARTIAL parity outcome to FULL/NEAR by finishing the §5 Declaration + §6 Object Management shim-throws (~48 `m17NotWired*_` sites in `RTIambassadorImpl.cpp` §5 + §6 collapse to ~0). Installs the `DLCFederateAmbassadorBridge` (M34-AD) at `connect()` time so subscriber-leg callbacks route through to the federate. Lands catalogue §16 MOM behavioral (real HLAobjectRoot.HLAmanager.* pub/sub). `[[deprecated]]` annotations on all `cppsdk/include/rti1516e/*.h` M17-era headers. `docs/MIGRATION_M17_TO_DLC.md` published (worked-example port from M17 `rti1516e::RtiAmbassador` → DLC `RTI::RTIambassador`). New `tests/conformance/rti/ivct-subset/` — gorti rtid passes an IVCT-derived conformance subset per `docs/RTI_CONFORMANCE_AUDIT.md §6` (initial catalog: 3+ tests covering §4 federation + §5 pub/sub + §6 object exchange). **Final parity outcome:** *M35-PARITY-OUTCOME: FULL MATCH — 9/9 publisher + 7/7 subscriber events byte-identical to Pitch pRTI Free 5.5.10 on `om_helloworld_pubsub` after §5.2.1 canonicalization (RO-bucket sort of RECEIVE/REFLECT). Verified against live rtid on integrated main 2026-07-02; canonicalized captures at `gorti-captured.{publisher,subscriber}.log`.* Post-fan-out orchestrator reconciliation completed the 3-way BB+BC+BH M17Bridge merge (§5+§6+§10 impls + evoke wiring + BD's bridge install at connect/disconnect) and fixed two fixture emission bugs (bare `SUB: RESIGN`, default-precision REFLECT doubles). Zero regressions: lockfile 40/0, runtime GREEN suites 5+22+36 all PASS. **DONE 2026-07-02** — see M35-{BA,BB,BC,BD,BE,BF,BG,BH} merges + reconciliation commits. |
| **M36** | Agents DA/DB/DC/DD | DLC parity wave 2 — drive the 27-fixture scoreboard from ≈87% to ≈94% with ZERO proto changes | FR-DLC-14 §10.4 re-entrancy guard (`gorti::dlc::CallbackScope`), cppsdk `removeObjectInstance` wire→callback, provideAttributeValueUpdate path, lookahead floor fix, DDM-aware DISCOVER fan-out (`subscribersForDiscover`), restore-by-name, MOM instance fan-out through the standard registry path, pysdk resign-action discard fix (DD-1), own_* golden reviews. Newly FULL/SPEC-FULL: tm_lookahead_change 16/16, ddm_region_overlap 24/24, om_request_attribute_update_{class,instance}, threading_callback_reentry 7/7, xlang_python_cpp_pubsub 17/17, mom_federation_lifecycle 18/18 — **14 of 27 fixtures FULL/SPEC-FULL, ≈432/460 events (≈94%), 0 BLOCKED**. Remaining gaps pinned as the M37 backlog (proto slots + server order/semantics + DLC callback deferral). **DONE 2026-07-02** — see M36-{DA,DB,DC,DD} merges. |
| **M37** | Agents EA/EB/EC/ED/EF/EE | Close the pinned M36 backlog end-to-end (proto verticals + server semantics + DLC bridge) and re-verdict all 27 DLC conformance fixtures — the M31→M37 compliance program's closing milestone | **EA**: additive proto slots for every pinned wire gap — §4.12 sync-registration ack, §4.14 successfully=false, §4.15 failedToSyncSet, §4.25/§4.26 restore events + InitiateFederateRestore.federate_name, §5.10-13 startRegistration, §6.17-18 scope advisories, §7.11 release-request, §8.22 requestRetraction with real retraction handles, §7.9 if_available flag, §7.6 ConfirmDivestiture RPC. **EB**: server §8.14 drain-buffered-TSO-before-grant, outgoing-TSO timestamp validation (EB-3), late-join retroactive DISCOVER (EB-4), TAR grants at exact target (EB-5), delete fan-out via subscribersForDiscover (EB-1). **EC**: 16 DLC bridge converters for the new wire, real §7.6 two-phase + §7.9 atomic if_available at the DLC, deferred synthesized-callback queue (kills every answer-before-call strict swap), DLC-boundary typed error sniffs (bridge suite grows to 53). **ED**: deterministic scripted driver `_harness/run_fixture.sh` + per-fixture `driver.conf`, §8.1.2 tm_ner fixture legality fix + golden amendments, own_query golden review. **EF**: tm_ner 'regression' triaged to a stale fixture binary (conf targets are EXCLUDE_FROM_ALL); grant-targeting invariant pinned in rti/spec/M37. **EE (final verdict, this row)**: all 27 fixtures re-run on one integrated build — **26/27 FULL/SPEC-FULL, 459/459 golden events matched (100%; strict-parity 459/460 ≈ 99.8% counting the sole residual: tm_tso_ordering's extra interim GRANT, a benign-liveness NER divergence pinned to rti/internal/time/ner.go)**, up from ≈94% (M36) / ≈87% (M35). Newly FULL this milestone: fm_sync_full 20/20, fm_sync_subset_with_failure 17/17, fm_save_restore_roundtrip 18/18, dm_pub_sub_active_passive 11/11, dm_unpublish_whole_vs_attrs 9/9 (EE sniff-order one-liner), om_message_retraction 15/15, own_release_request_denied 11/11, own_query_via_callbacks 14/14 strict, own_acquire_if_available_race 17/17 strict (real deny-fast; stale late-notification bug gone), own_negotiated_divest re-confirmed on the REAL two-phase, ddm_region_mod_in_flight 28/28, tm_ner_pair 30/30. Definitive scoreboard + honest FULL-semantics statement in `docs/PITCH_PARITY.md` (only om_helloworld_pubsub, fm_list_executions, fm_create_join_resign §1-2 have Pitch-captured goldens; the rest are spec-derived). driver.conf committed for all 25 driver-able fixtures; rtid cwd isolated per-run (stale savepoint bundle broke back-to-back save/restore). Gates: `go test ./rti/...` PASS, C++ runtime suites 53+36+5+1 PASS, lockfile 40/40, spec-traceability 28/28 GREEN (EE added the missing `_runtime` README); pre-existing on main and unchanged: callbacks_toggle_integration 1 M17-era failure (EvokeCallbackDispatchesExactlyOnePerCall). **DONE 2026-07-02** — see M37-{EA,EB,EC,ED,EF,EE} merges. |
...
## 2026-07-02 (M35 — in progress; §5+§6 real impls + MOM + [[deprecated]] on M17 rti1516e headers + IVCT-derived subset)

Fifth (final) milestone of the DLC compliance track. Drives the M34 PARTIAL parity result on `om_helloworld_pubsub` to FULL/NEAR by finishing:

- §5 Declaration Management real impls — the 10 shim-throws at `cppsdk/src/dlc/RTIambassadorImpl.cpp` `m17NotWiredDM_(...)` sites collapse to actual M17 `declaration.Manager` calls (publish/subscribe object class + attributes, publish/subscribe interaction class, un* forms).
- §6 Object Management real impls — the ~38 shim-throws at `m17NotWired_(...)` sites collapse to real M17 `object.Registry` calls (reserve/release name, register/update/delete/local-delete, send interaction, request update, change transportation/order).
- `DLCFederateAmbassadorBridge` installed at `connect()` time (M34-AD landed the bridge type; M35 wires `callback_bridge_ = std::make_unique<...>(...)` in `RTIambassadorImpl::connect`). Unblocks the subscriber-leg discover / reflect / receive / remove callbacks — the last gap between the M34 PARTIAL 2/7 subscriber match and a byte-identical Pitch parity.
- §16 MOM behavioral — real `HLAobjectRoot.HLAmanager.*` publish/subscribe wiring so the DLC C++ surface exposes MOM parity with the pysdk M20 surface.
- `[[deprecated]]` annotations on **every** `cppsdk/include/rti1516e/*.h` M17-era header (5 headers: `Encoding.h`, `Exceptions.h`, `FederateAmbassador.h`, `RtiAmbassador.h`, `Types.h`). M17 stays functionally live but compiler-visible steers new federate code onto the strict DLC `RTI/` surface.
- `docs/MIGRATION_M17_TO_DLC.md` published — worked-example port of a federate from `rti1516e::` to `RTI::` (namespace + factory + wstring conversion patterns).
- `tests/conformance/rti/ivct-subset/` — first checked-in IVCT-derived conformance subset covering §4 federation + §5 pub/sub + §6 object exchange per `docs/RTI_CONFORMANCE_AUDIT.md §6`. Initial catalog: ≥3 tests. gorti rtid PASSES the subset (the conformance gate that closes Cut-4).

Seven-agent fan-out (BA/BB/BC/BD/BE/BF/BG):

- **Agent BA** — §5 Declaration Management real impls (10 methods; wires through to `declaration.Manager`).
- **Agent BB** — §6 Object Management real impls (~38 methods; wires through to `object.Registry` + reservation table).
- **Agent BC** — `DLCFederateAmbassadorBridge` install at connect() + subscriber callback fanout wiring.
- **Agent BD** — §16 MOM behavioral: HLAobjectRoot.HLAmanager.* pub/sub via existing M17 MOM transport.
- **Agent BE** — Real gorti↔Pitch parity diff run on `om_helloworld_pubsub`; backfills M35-PARITY-OUTCOME sentinel.
- **Agent BF** — `[[deprecated]]` on M17 headers + `docs/MIGRATION_M17_TO_DLC.md` publish + `tests/conformance/rti/ivct-subset/` build-out.
- **Agent BG** — Acceptance gate / CHANGELOG / PITCH_PARITY / check_m35 probe / spec-coverage refresh / memory rollup (this entry).

Acceptance: `scripts/check-milestones.sh check_m35` (6 probes covering §5+§6 shim-throw count drop, DLCFederateAmbassadorBridge install at connect, `[[deprecated]]` M17 header markers, IVCT-subset presence, M34-PARITY-OUTCOME sentinel promoted to FULL/NEAR, CHANGELOG M35 row). Spec-coverage matrix refreshed by `scripts/gen-spec-coverage.sh`.

Final M35 count + close date + M35-PARITY-OUTCOME sentinel filled in by BG after BA/BB/BC/BD/BE/BF merge to main.

---

## 2026-07-02 (M34 — DONE; §4 pull-forward + §5 shim + §8 Time Mgmt + encoding refinements + FederateAmbassadorBridge + first REAL gorti↔Pitch parity: PARTIAL)

Fourth milestone of the DLC compliance track. Two-thread scope:

**Thread 1: §4 / §5 / §8 pull-forward** — the M33 orchestrator-dispatch gap left §4 Federation Management as pure `m32_stub` throws, blocking any conformance fixture at `connect()`. M34-AA replaces the 25 §4 sites in `RTIambassadorImpl.cpp` with real M17 wire-through — createFederationExecution / joinFederationExecution / resignFederationExecution / connect / disconnect / save+restore lifecycle. M34-AC lands §5 Declaration Management wstring↔M17 shim (10 catalogue rows; the ~10 remaining shim-throws at `m17NotWiredDM_` are M35 scope). M34-AB lands §8 Time Management (13 catalogue rows §9.1-§9.14 — enable/disable regulating+constrained, TAR/TARA/NER/NMRA/FQR, queryLogicalTime/GALT/LITS/Lookahead, timeAdvanceGrant dispatch, LBTSResult struct).

**Thread 2: Original M34 encoding + time types + callback bridge** — M34-AF lands §14 encoding refinements (composites + round-trip test, 36 gtests). M34-AE fills `HLAfloat64Time` / `HLAinteger64Time` full behavioral impl (federation chooses via `createFederationExecution(..., logicalTimeImplementationName=L"HLAfloat64Time")`). **M34-AD lands `cppsdk/src/dlc/FederateAmbassadorBridge.{h,cpp}`** — the callback dispatch bridge from gorti M17 event stream to DLC `FederateAmbassador` interface (22 gtests). The bridge is the type; **M35 wires it into `RTIambassadorImpl::connect()` at `callback_bridge_ = std::make_unique<...>(...)` to actually route subscriber-leg callbacks.**

**M34-AG acceptance-gate integration surfaced two real gorti M17 bugs that also landed here:**
- `rti/pkg/fom/parser/strict.go` — added the 4 spec-mandated advisory switches per IEEE 1516.2-2010 §F.10 (`attributeScopeAdvisory`, `attributeRelevanceAdvisory`, `objectClassRelevanceAdvisory`, `interactionRelevanceAdvisory`). gorti's strict FOM parser was rejecting the standard-form FOM Pitch ships; this is a real gorti M17 compliance gap surfaced by the DLC track.
- `cppsdk/src/dlc/BasicDataElements.cpp` — added 19 missing `set()` bodies for the `DEFINE_ENCODING_HELPER_CLASS` vtables. Agent F (M32) landed the spec-mandated `set()` declarations in the header without paired impls; fixture link failed until M34-AG fixed.

**First REAL gorti↔Pitch parity diff on `om_helloworld_pubsub`: `M34-PARITY-OUTCOME: PARTIAL 2/9 pub + 2/7 sub`**. Historic result — first checked-in evidence of the strict DLC surface producing spec-canonical output. Publisher matches Pitch byte-identical on CONNECT + JOIN (2/9 events); CREATE is a fixture emission gap; PUBLISH + REGISTER + UPDATE + SEND + RESIGN pending §5+§6 impl (remaining `m17NotWired*_` shim-throws). Subscriber matches on CONNECT + JOIN (2/7); SUBSCRIBE + DISCOVER + RECEIVE + REFLECT + RESIGN pending §5+§6 impl + `DLCFederateAmbassadorBridge` install at connect. `gorti-captured.publisher.log` + `gorti-captured.subscriber.log` committed alongside Pitch goldens under `cppsdk/tests/dlc/conformance/om_helloworld_pubsub/`.

Seven-agent fan-out (AA/AB/AC/AD/AE/AF/AG). Acceptance: `scripts/check-milestones.sh check_m34` (7 probes). Spec-coverage matrix refreshed by `scripts/gen-spec-coverage.sh`.

Out of scope (defer to M35):
- §5 Declaration Management remaining `m17NotWiredDM_` shim-throws (M35 collapses these — closes catalogue §5.2/§5.3/§5.5/§5.6/§5.8/§5.9/§5.11 wire-through).
- §6 Object Management remaining `m17NotWired_` shim-throws (~38 sites — M35 collapses).
- `DLCFederateAmbassadorBridge` install at `RTIambassadorImpl::connect()` (M35).
- §16 MOM behavioral (M35).
- `[[deprecated]]` on `cppsdk/include/rti1516e/*.h` M17 headers (M35).
- `docs/MIGRATION_M17_TO_DLC.md` publish (M35).
- IVCT-derived conformance subset (M35).
