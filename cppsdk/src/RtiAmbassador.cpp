...
// M39 — metadata-first spec-exception dispatch.
//
...
class RTIambassadorImpl {
 public:
  ...
  // §8.21/§8.22 (M37) — per-federate monotonic
...
// M35 — explicit destructor: stop the background event stream
...
void M17RTIambassador::resignFederationExecution() {
  ...
  // Cut-1: default action mirrors reference_rti's pre-M24 default. The wider
...
// §4.14 (M37) — achieve with the explicit `successfully` flag.
...
void M17RTIambassador::negotiatedAttributeOwnershipDivestiture(
  ...
  fillObjectAttrsReq(req, impl_->joined_federation,
                     impl_->federate_handle.raw(), object.raw(), attributes);
  req.set_tag(tag.data(), tag.size());
  // M37 — additive §7.6 flag; old servers ignore it (one-phase).
...
// §7.6 (M37) — complete a two-phase negotiated divestiture.
...
// §7.9 (M37) — same RPC with the additive if_available flag;
...
// --- M36 — §6 TSO variants + delete / request-update wire ---------
...
// §8.21/§8.22 (M37) — TSO sends carrying a freshly-allocated
...
// --- M36 — §10.24/§10.25 federate name<->handle -------------------
...
bool RTIambassadorImpl::dispatchOneEvent() {
  ...
  switch (evt.event_case()) {
    ...
    case rti::v1::FederateEvent::kSyncSynchronized: {
      // §4.15 — M37: dispatch the failed-set-carrying slot;
    ...
    case rti::v1::FederateEvent::kSyncRegistrationFailed: {
      // §4.12 — M37. Proto enum values mirror the M17 enum.
    ...
    case rti::v1::FederateEvent::kRestoreInitiate: {
      ...
      // M37: dispatch the name-carrying §4.26 slot; its default
    ...
    case rti::v1::FederateEvent::kRemove: {
      // §6.15 — M36. Timestamp presence selects RO vs TSO at
...
bool M17RTIambassador::evokeCallback(double approx_min_time,
  ...
  // Hold off dispatch until min_deadline elapses (reference_rti contract:
  ...
  // signal reference_rti federates use to keep looping.
