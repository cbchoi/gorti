// M34 — DLCFederateAmbassadorBridge impl.
//
...
// M37 — §4.15 failedToSyncSet conversion.
...
// M37 — §4.12 failure-reason enum mapping. See header comment
...
// ===== FR-DLC-14 re-entrancy witness (M36) =====
...
void DLCFederateAmbassadorBridge::reflectAttributeValues(
  ...
  if (timestamp.has_value()) {
    ...
    // does not retain a pointer. M36: reference_rti delivers TSO through
...
void DLCFederateAmbassadorBridge::receiveInteraction(
  ...
  if (timestamp.has_value()) {
    // M36: TSO delivery uses the 9-arg retraction-handle
    // overload (IEEE 1516.1-2010 API shape) — see reflectAttributeValues note above.
    rti1516e::HLAfloat64Time t(*timestamp);
...
void DLCFederateAmbassadorBridge::removeObjectInstance(
  ...
  if (timestamp.has_value()) {
    // §6.15 TSO — retraction-handle overload (IEEE 1516.1-2010 API shape; the wire has
...
void DLCFederateAmbassadorBridge::federationSynchronized(
  ...
  // failedToSyncSet parameter; M37 wire).
...
void DLCFederateAmbassadorBridge::attributeOwnershipUnavailable(
  ...
  if (!dlc_fed_) return;
  CallbackScope scope;  // FR-DLC-14 — mark the callback context
  // §7.10 — real wire delivery (M37 emits on the if_available
...
void DLCFederateAmbassadorBridge::initiateFederateRestore(
  ...
  if (!dlc_fed_) return;
  CallbackScope scope;  // FR-DLC-14 — mark the callback context
  // §4.26 — 3-arg M17 slot (M37 wire): the save-time federate
  // name forwards for real. M37.
  dlc_fed_->initiateFederateRestore(
