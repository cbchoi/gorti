...
// M36 — listFederationExecutions dials its own channel (the M17
...
struct M17Bridge::Impl {
  ...
  // renamed it from `RTIambassador` to `M17RTIambassador` so the
  ...
  // M36 — dial URL recorded at connect() so
...
auto guard(char const* op, Fn&& f) -> decltype(f()) {
  try {
    ...
    // M39 — structured spec-exception carrier. The M17 client
...
// ---------- §6 delete / request-update + TSO variants (M36) -------
...
void M17Bridge::confirmDivestiture(std::uint64_t object,
                                   const std::vector<std::uint64_t>& attrs) {
  // M37 EC-2 — real §7.6 ConfirmDivestiture RPC (M37 wire).
