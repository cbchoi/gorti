...
// label, FOM path) are ASCII in the retained verification fixtures; the shim tolerates
...
std::string ws2s(std::wstring const& w) {
  ...
  for (wchar_t c : w) {
    ...
    // IEEE FOM conventions. Full UTF-8 encode is FR-DLC-17 M35 territory.
...
// Parse §4.2 `localSettingsDesignator`. gorti accepts its
// "gortiAddress=host:port" option or a bare "grpc://host:port" or
...
// scheme is missing, defaults to "grpc://127.0.0.1:8080" (matches the M17
// documented default; the fixture wire tests write ":8080" too).
std::string parseLocalSettings(std::wstring const& settings) {
  ...
  if (lstrip == std::string::npos) return "grpc://127.0.0.1:8080";
  s = s.substr(lstrip, rstrip - lstrip + 1);

  // Direct grpc scheme — pass through.
  if (s.rfind("grpc://", 0) == 0 || s.rfind("grpcs://", 0) == 0) return s;

  // Look for gorti's implementation-defined address option.
  auto const kKey = std::string("gortiAddress=");
  auto pos = s.find(kKey);
  if (pos != std::string::npos) {
    // Value runs from after "=" to the next comma or end.
    ...
    if (hl == std::string::npos) return "grpc://127.0.0.1:8080";
...
// matching <RTI/Exception.h> type. M39: extracted to
...
// FR-DLC-14 (M36) — §10.4 re-entrancy gate. A federate must not
...
// M35 — §6 handle adapters. Same shape as the FederateHandle
...
// M35 — §10 support-service adapters. `make*FromUint64` widens
...
// M36 — §8/§9 handle adapters. Same 8-byte big-endian VLD shape.
...
// M37 — widen M17's retractable-send return (raw uint64) into
...
//
// M34 — wstring-adapter shim over the M17 concrete
...
// §4.2 — connect. Parses gorti's `localSettingsDesignator` option
// "gortiAddress=host:port" or a bare "grpc://host:port" into an M17 dial URL.
// Stores the FederateAmbassador reference and CallbackModel for the
// callback dispatch bridge. The bridge owns callback binding;
...
// callback deliveries can no-op safely ( checks fed_amb_ before
// dispatching).
...
// §4.7 — list federation executions. M36: the wire's
...
// (IEEE 1516e 2-arg shape). M17 accepts (federate_name, federation).
...
// flag-carrying overload (M37 wire) — false still counts toward
...
// DLC callback signal is deferred to the dispatch bridge.
...
//
// M35: real M17 delegation replacing M34 shim.
//
...
// ===== §6 Object Management =====
//
// M35: real M17 delegation via M17Bridge, replacing the M33 shim throws.
// It uses the same wstring-adapter shape as §4/§5/§8:
//   - wstring → narrow via ws2s (top-of-TU helper); DLC accepts wide, M17
//     speaks UTF-8 narrow. Names are ASCII in FOM XML per IEEE 1516 API convention.
...
MessageRetractionHandle DLCRTIambassadorImpl::updateAttributeValues(
    ...
    VariableLengthData const& theUserSuppliedTag,
    LogicalTime const& theTime) {
  // §6.10 overload 2 (TSO) — M36: real timed wire; M37 EC-2:
...
MessageRetractionHandle DLCRTIambassadorImpl::sendInteraction(
    ...
    VariableLengthData const& theUserSuppliedTag,
    LogicalTime const& theTime) {
  // §6.12 overload 2 (TSO) — M36: real timed wire; M37 EC-2:
...
void DLCRTIambassadorImpl::deleteObjectInstance(
    ObjectInstanceHandle theObject,
    VariableLengthData const& theUserSuppliedTag) {
  // §6.14 overload 1 (RO) — M36: real M23 DeleteObjectInstance
...
MessageRetractionHandle DLCRTIambassadorImpl::deleteObjectInstance(
    ...
    VariableLengthData const& theUserSuppliedTag,
    LogicalTime const& theTime) {
  // §6.14 overload 2 (TSO) — M36: timed wire; subscribers see
...
void DLCRTIambassadorImpl::requestAttributeValueUpdate(
    ...
    VariableLengthData const& theUserSuppliedTag) {
  // §6.19 overload 1 — by object instance. M36: real M23
...
void DLCRTIambassadorImpl::requestAttributeValueUpdate(
    ...
    AttributeHandleSet const& theAttributes,
    VariableLengthData const& theUserSuppliedTag) {
  // §6.19 overload 2 — by object class. M36: real M23
...
//
// M36: real M17 delegation via M17Bridge (replaces M33-K's
...
// EC-2: real ConfirmDivestiture RPC via M17 (M37 wire) — replaces
...
// ) — replaces CA's query-then-acquire emulation. The server
...
void DLCRTIambassadorImpl::queryAttributeOwnership(
  ...
  // evoke so the callback follows the call's return (reference RTI ordering).
  auto* fed = fed_amb_;
...
// ===== §8 Time Management =====
//
// M36: real M17 delegation via M17Bridge, replacing the earlier
// m17NotWiredTime_ throws. Design:
...
void DLCRTIambassadorImpl::enableTimeRegulation(
  ...
  if (fed_amb_ != nullptr) {
    // M37 EC-4 — deliver the synthesized ack on the next evoke (reference RTI
...
// ===== §9 DDM =====
//
// M36: real M17 delegation via M17Bridge (replaces the M32
// stubs). Shape notes:
...
// ===== §10 Support Services =====
//
// M33 impl. Design: the DLC RTIambassador has no gRPC binding yet —
...
ResignAction DLCRTIambassadorImpl::getAutomaticResignDirective() {
  // §10.2 — the compatibility baseline is NO_ACTION; return that until a real
...
FederateHandle DLCRTIambassadorImpl::getFederateHandle(
  ...
  // §10.4 — M36: real resolution via the M24
...
std::wstring DLCRTIambassadorImpl::getFederateName(FederateHandle theHandle) {
  // §10.5 — M36: reverse lookup over the same member list.
...
// M35 — §10 support services now delegate to M17 for real
...
AttributeHandle DLCRTIambassadorImpl::getAttributeHandle(
  ...
  // §10.11 — M35 real delegation. Class-handle validation
...
DimensionHandle DLCRTIambassadorImpl::getDimensionHandle(
    std::wstring const& theName) {
  // §10.25 — M36 real delegation. 1516e dimensions live in
...
RangeBounds DLCRTIambassadorImpl::getRangeBounds(RegionHandle theRegion,
  ...
  // §10.29 — M36 real delegation via M17 queryBounds. A region
...
void DLCRTIambassadorImpl::setRangeBounds(RegionHandle theRegion,
  ...
  // §10.30 — M36 real delegation. Validate arguments per spec
...
bool DLCRTIambassadorImpl::evokeCallback(double approximateMinimumTimeInSeconds) {
  ...
  // synthesis sites) deliver FIRST, one per evoke, matching the reference RTI's
...
// §10.43-10.44 enable/disableCallbacks — M35: delegate to M17
...
// §10 (catalogue 13.15) — 9 decode*Handle methods. All are `const` after
// the M32 drift fix. Each Friend shim invokes the handle's
...
// M34 — bodies for the FederateHandle adapters declared at the
...
// M35 — §6 handle adapters. Same 8-byte big-endian encoding as
...
// M35 — §10 support-service adapters bodies. Uses same 8-byte
...
// M37 — retractable-send return widening.
