...
// M35 — forward-decl of the callback bridge (full defn lives in
...
class DLCRTIambassadorImpl : public RTIambassador {
 public:
  ...
  // §10 — IEEE 1516.1-2010 defines this accessor as const.
  rti1516e::auto_ptr<LogicalTimeFactory> getTimeFactory() const override;

  // §10 — 9 decode*Handle methods. All `const` per reference_rtiambassador.h:1776-1846.
  ...
  // RTIs (reference_rti) deliver them after the call returns, on the next
 ...
 private:
  ...
  // M37 — §10.35/§10.36 attribute-scope-advisory switch,
  ...
  // Bound DLC-side FederateAmbassador reference from §4.2 connect(). The
  // callback dispatch bridge reads this to route M17 callbacks through
  ...
  // to match the reference_rti pre-connect state (safe: no callbacks fire).
  CallbackModel callback_model_{HLA_EVOKED};

  // M36 — cached handle of gorti's implicit DDM routing space
  ...
  // M35 — DLC-side callback dispatch bridge. Owns an
  // rti1516e_m17::FederateAmbassador subclass ( bridge) that
