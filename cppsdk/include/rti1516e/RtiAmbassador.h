...
//
// Shape mirrors reference_rti's `rti1516e::RTIambassador` so a federate ported
// from reference_rti / Portico / MAK can recompile against this header with
// minimal call-site change. See docs/COMMERCIAL_RTI_PARITY.md for the
// divergence table.
...
// M35 — M17 shim deprecation gate. See Types.h for the pattern;
...
// M34 — renamed from `RTIambassador` to `M17RTIambassador` so the
...
class M17RTIambassador {
 public:
  ...
  //
  // Cut-1 default action: UNCONDITIONALLY_DIVEST_ATTRIBUTES (reference_rti's
  ...
  // §6.1-5 — object instance name reservation flow. reference_rti federates
  ...
  //
  // IEEE 1516.1-2010 API shape: handles only. The federate must have resolved the
  ...
  // ===== M36 — timed (TSO) §6 variants + §6.14/§6.19 wire =====
  //
  ...
  // §8.21/§8.22 (M37) — TSO sends that allocate and return a
  ...
  // ``two_phase`` (M37 , default false = pre-M37 one-phase):
  ...
  // requestDivestitureConfirmation callback. M37.
  ...
  // the §4.15 federationSynchronized failed-to-sync set. M37.
  void synchronizationPointAchieved(const std::string& label,
                                    bool successfully);
  ...
  // Documented in docs/COMMERCIAL_RTI_PARITY.md.
  ...
  // §10.4 — reference_rti-name alias for tickCallback. Same semantic as
...
// M34 — reference_rti-parity compat alias. Every M17 test / federate
