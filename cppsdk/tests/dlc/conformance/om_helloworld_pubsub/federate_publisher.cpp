...
//
// M31 status: this translation unit parses against the RTI/*.h forward-declaration
...
int main() {
  try {
    ...
    try {
      ...
      // manager's federation registry) may have created it. A conforming RTI
      // commercial RTI's golden trace, `PUB: CREATE` is unconditional — the spec
