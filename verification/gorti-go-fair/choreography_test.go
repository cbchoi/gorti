...
func (fake *choreographyFederate) QueueAttributeValuesByHandle(
	_ context.Context, _ uint64, attributes map[uint64][]byte,
) (uint64, error) {
	if len(attributes) != 2 || attributes[31] == nil || attributes[32] == nil {
		return 0, fmt.Errorf("attribute handles = %v, want 31 and 32", attributes)
	}
	fake.calls = append(fake.calls, "queue-update")
	return uint64(len(fake.calls)), nil
}

func (fake *choreographyFederate) QueueInteractionByHandle(
	_ context.Context, classHandle uint64, parameters map[uint64][]byte,
) (uint64, error) {
	if classHandle != 17 {
		return 0, fmt.Errorf("interaction class handle = %d, want 17", classHandle)
	}
	if len(parameters) != 2 || parameters[21] == nil || parameters[22] == nil {
		return 0, fmt.Errorf("parameter handles = %v, want 21 and 22", parameters)
	}
	fake.calls = append(fake.calls, "queue-interaction")
	return uint64(len(fake.calls)), nil
}

func (fake *choreographyFederate) FlushLocalLRC(context.Context) error {
	fake.calls = append(fake.calls, "flush")
	return nil
}
...
func TestPublisherStagesTimestampedTrafficBeforeLockstepAdvances(t *testing.T) {
	...
	if err := p.advancePublishedTimestampOrderTraffic(context.Background(), fake, 9); err != nil {
		t.Fatal(err)
	}
	want := []string{
		"update:1", "interaction:1",
		"update:2", "interaction:2",
		"tar:1", "tar:2",
		"delete:3", "tar:3",
	}
...
func TestPublisherLocalLRCPreservesOperationOrderAndFlushes(t *testing.T) {
	cfg := config{
		Role: rolePublisher, Seed: "1516", Count: 2, Timeout: time.Second,
		ReceiveOrder: true, LocalLRC: true,
	}
	payloads, err := preencodeWorkload(cfg.Seed, cfg.Count)
	if err != nil {
		t.Fatal(err)
	...
	fake := &choreographyFederate{}
	p := participant{
		cfg: cfg, log: testRunLog(t), payloads: payloads, interactionClassHandle: 17,
	}
	if err := p.publishTraffic(context.Background(), fake, 9); err != nil {
		t.Fatal(err)
	}
	want := []string{
		"queue-update", "queue-interaction",
		"queue-update", "queue-interaction",
		"flush",
	}
	if !reflect.DeepEqual(fake.calls, want) {
