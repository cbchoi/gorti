...
func TestIeee1516CompatibleHLAEncodings(t *testing.T) {
	...
	text, err := encodeASCIIString("IEEE1516")
	if err != nil {
		t.Fatal(err)
	}
	if got := hex.EncodeToString(text); got != "000000084945454531353136" {
		t.Fatalf("string encoding = %s", got)
	}
	decodedText, err := decodeASCIIString(text)
	if err != nil || decodedText != "IEEE1516" {
