...
it can be byte-compared with a Pitch run. Timing measurements are emitted to a
separate NDJSON stream with the same metric schema used by the Pitch verifier.
"""
...
DEFAULT_FEDERATION = "GortiPitchVerifier-20260712"
...
OBJECT_NAME = "PitchVerifierEntity"
...
def deterministic_payload(seed: int, channel: str, index: int) -> str:
    """Return the shared Pitch/gorti fixed-seed payload representation."""
...
class PerformanceLog:
    """Collect durations and render Pitch-compatible aggregate metrics."""
...
class VerificationRunner:
    ...
    def _execute(self) -> None:
        ...
        self._call(
            ...
            {"class": OBJECT_CLASS, "attributes": ["Sequence", "Payload"]},
        )
        self._call(
            "DM",
        ...
        self._call(
            ...
            "producer",
            lambda: self.producer.reserveObjectInstanceName(OBJECT_NAME),
            {"name": OBJECT_NAME},
        )
        self._wait_until(
            lambda: (
                OBJECT_NAME in self.producer.state_snapshot()[3]
                or OBJECT_NAME in self.producer.state_snapshot()[4]
            ),
            "object name reservation",
        )
        if OBJECT_NAME not in self.producer.state_snapshot()[3]:
        ...
        instance = self._call(
            ...
            lambda: self.producer.registerObjectInstance(
                producer_handles["object_class"], OBJECT_NAME
            ),
            {"class": OBJECT_CLASS, "name": OBJECT_NAME},
        )
        ...
        self._call(
            ...
            {"name": OBJECT_NAME, "logical_time": int(removal_time)},
        )
    ...
    def _handles(self, federate: Rti1516eAmbassador) -> dict[str, int]:
        object_class = federate.getObjectClassHandle(OBJECT_CLASS)
        interaction_class = federate.getInteractionClassHandle(INTERACTION_CLASS)
        return {
    ...
    def _review(self) -> None:
        ...
        discovered_handle = next(
            (
                ...
                if class_handle == self.consumer.expected_object_class and name == OBJECT_NAME
            ),
            None,
        )
        ...
        reservation_ok = OBJECT_NAME in reservations_ok and OBJECT_NAME not in reservations_failed
