...
if (-not [string]::IsNullOrWhiteSpace($Workload.object_class)) {
    $ObjectClass = [string]$Workload.object_class
}
if (-not [string]::IsNullOrWhiteSpace($Workload.interaction_class)) {
    $InteractionClass = [string]$Workload.interaction_class
}
if (-not [string]::IsNullOrWhiteSpace($Workload.object_name)) {
    $ObjectName = [string]$Workload.object_name
}
...
$ExactArgv = @(
    ...
    '-ObjectClass', $ObjectClass, '-InteractionClass', $InteractionClass,
    '-ObjectName', $ObjectName
)
if (-not [string]::IsNullOrWhiteSpace($ServerAddress)) {
    $ExactArgv += @('-ServerAddress', $ServerAddress)
}
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$ArgvPath = Join-Path $OutputDirectory 'reference_rti-exact-argv.json'
...
foreach ($Binding in @(
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$Binding[1])) {
        $RunArguments += $Binding
    }
}
foreach ($Argument in $LauncherArgument) {
    $RunArguments += @('-LauncherArgument', $Argument)
}
& powershell @RunArguments
if ($LASTEXITCODE -ne 0) {
    throw "reference_rti fair run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "reference_rti result construction failed with exit code $LASTEXITCODE."
}
...
if (Test-Path -LiteralPath $Check -PathType Leaf) {
    & $PythonExecutable $Check result $ResultPath --expected-workload $WorkloadContract `
        --implementation reference_rti --run-id $RunId
    if ($LASTEXITCODE -ne 0) {
        throw 'reference_rti result failed the shared fair-comparison contract.'
    }
}

Write-Host "reference_rti fair result: $ResultPath"
