...
IMPLEMENTATIONS: Final = ("reference_rti", "go")
ORDERS: Final = ("AB", "BA")
...
def validate_workload(value: object, path: str = "workload") -> dict[str, Any]:
    ...
    _exact_keys(workload, path, required, {"object_class", "interaction_class", "object_name"})
    ...
    if "object_class" in workload:
        _string(workload["object_class"], f"{path}.object_class")
    if "interaction_class" in workload:
        _string(workload["interaction_class"], f"{path}.interaction_class")
    if "object_name" in workload and workload["object_name"] is not None:
        _string(workload["object_name"], f"{path}.object_name")
...
def validate_manifest(value: object, *, require_complete: bool = True) -> dict[str, Any]:
    ...
    for index, item in enumerate(pairs):
        ...
        implementations = ("reference_rti", "go") if order == "AB" else ("go", "reference_rti")
    ...
    for index, item in enumerate(runs):
        ...
        expected_implementation = ("reference_rti", "go") if order == "AB" else ("go", "reference_rti")
