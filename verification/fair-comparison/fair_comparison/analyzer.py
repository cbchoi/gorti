...
def _aggregate_accounting(measured: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_implementation: dict[str, dict[str, int]] = {
        implementation: dict.fromkeys(fields, 0) for implementation in ("reference_rti", "go")
    }
...
def analyze_manifest(
    ...
    *,
    ...
    if any(set(arms) != {"reference_rti", "go"} for arms in by_pair.values()):
        raise ContractError("a measured pair is missing reference_rti or Go")
    ...
    for metric_index, metric_key in enumerate(metric_keys):
        ...
        for statistic_index, statistic in enumerate(STATISTICS):
            ...
            arm_values: dict[str, list[float]] = {"reference_rti": [], "go": []}
            by_order: dict[str, list[float]] = {"AB": [], "BA": []}
            for pair_index in sorted(by_pair):
                arms = by_pair[pair_index]
                reference_rti_value = float(arms["reference_rti"]["metrics"][metric_key][statistic])
                go_value = float(arms["go"]["metrics"][metric_key][statistic])
                if reference_rti_value <= 0 or go_value <= 0:
                    raise ContractError(
                        f"paired ratios require positive {statistic} values for {metric_key}"
                    )
                ratio = go_value / reference_rti_value
                order = str(arms["reference_rti"]["order"])
                ratios.append(ratio)
                by_order[order].append(ratio)
                arm_values["reference_rti"].append(reference_rti_value)
                arm_values["go"].append(go_value)
                ratio_records.append(
                    {"pair_index": pair_index, "order": order, "go_over_reference_rti": ratio}
                )
            ...
            order_effect: dict[str, Any] = {
                ...
                "AB_median_go_over_reference_rti": statistics.median(by_order["AB"]),
                "BA_median_go_over_reference_rti": statistics.median(by_order["BA"]),
            }
            effect = (
                order_effect["AB_median_go_over_reference_rti"] / order_effect["BA_median_go_over_reference_rti"]
            )
            ...
            statistic_reports[statistic] = {
                "reference_rti_run_values": arm_values["reference_rti"],
                "go_run_values": arm_values["go"],
                "reference_rti_run_median": statistics.median(arm_values["reference_rti"]),
                "go_run_median": statistics.median(arm_values["go"]),
                "paired_ratios": ratio_records,
                "paired_median_go_over_reference_rti": statistics.median(ratios),
    ...
    measured_orders = [by_pair[index]["reference_rti"]["order"] for index in sorted(by_pair)]
