...
$CanonicalFom = if ([string]::IsNullOrWhiteSpace($Fom)) {
    [IO.Path]::GetFullPath(
        (Join-Path $RepoRoot 'verification\commercial-rti\fom\CommercialRtiVerifier.xml'))
} else {
...
try {
    foreach ($Pair in $SchedulePairs) {
        $Implementations = if ($Pair.order -eq 'AB') { @('reference_rti', 'go') } else { @('go', 'reference_rti') }
