...
$CanonicalFom = if ([string]::IsNullOrWhiteSpace($Fom)) {
    [IO.Path]::GetFullPath((Join-Path $RepoRoot 'verification\commercial-rti\fom\CommercialRtiVerifier.xml'))
} else {
    (Resolve-Path -LiteralPath $Fom).Path
}
if (-not (Test-Path -LiteralPath $CanonicalFom -PathType Leaf)) {
    throw "Reference RTI FOM not found at '$CanonicalFom'."
}
...
try {
    ...
    & (Join-Path $Root 'run-comparison.ps1') `
        ...
        -OutputDirectory $OutputDirectory -WarmupPairs $WarmupPairs -Fom $CanonicalFom `
        -MeasuredPairs $MeasuredPairs -OrderSeed $OrderSeed `
        -BootstrapResamples $BootstrapResamples -Python $Python `
        -ObjectClass $ObjectClass -InteractionClass $InteractionClass -ObjectName $ObjectName `
