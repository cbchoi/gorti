...
try {
    ...
    $LauncherArguments = @(
        ...
        '-InteractionClass', $Inputs.InteractionClass,
...
Invoke-FairConverter $Python $AdapterRoot $OutputDirectory $WorkloadContract $Fom `
    $Inputs.ObjectClass $Inputs.InteractionClass $Inputs.ObjectName 'go' $RunId
if ($null -ne $EventLogDescriptor) {
