...
function Assert-FairAdapterInputs(
    ...
    return [pscustomobject]@{
        ...
        FomSha256 = $FomHash
        ObjectClass = if ($Workload.object_class) { [string]$Workload.object_class } else { 'VerifierEntity' }
        InteractionClass = if ($Workload.interaction_class) { [string]$Workload.interaction_class } else { 'VerifierMessage' }
        ObjectName = if ($Workload.object_name) { [string]$Workload.object_name } else { 'CommercialRtiVerifierEntity' }
    }
}
