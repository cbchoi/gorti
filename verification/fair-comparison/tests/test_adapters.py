...
def canonical_records(count: int) -> list[dict[str, object]]:
    ...
    add("publisher", "OM", "object_registered", {"name": "PitchVerifierEntity"})
    add("subscriber", "OM", "object_discovered", {"name": "PitchVerifierEntity"})
...
def benchmark(shared: dict[str, object], implementation: str) -> dict[str, object]:
    actual_workload = {
        ...
        "seed": 1516 if implementation == "pitch" else "1516",
        "count": shared["count"],
        "two_process": True,
...
def test_real_artifact_adapters_produce_identical_semantic_projection(tmp_path: Path) -> None:
    pitch = convert_artifacts(**adapter_inputs(tmp_path, "pitch"))
    go = convert_artifacts(**adapter_inputs(tmp_path, "go"))

    assert pitch["semantics"] == go["semantics"]
    assert pitch["workload"] == go["workload"]
    assert pitch["accounting"]["expected_fanout"] == 4
    assert pitch["accounting"]["delivered"] == 4
    assert pitch["metrics"][0]["sample_scope"] == "subscriber_pre_tar_to_both_callbacks"
