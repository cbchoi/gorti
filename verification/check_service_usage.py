    ...
    for path in paths:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Attribute):
                ...
                if node.attr.endswith("Async"):
                    methods.add(node.attr.removesuffix("Async"))
...
def inspect(root: Path, language: str) -> dict[str, Any]:
    if language == "python":
        ...
        paths = [verifier] if verifier.is_file() else sorted(root.rglob("*.py"))
        methods = _python_methods(paths)
    else:
        source_root = root / "src"
        paths = sorted((source_root if source_root.is_dir() else root).rglob("*.java"))
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        dest="reference_rti",
        type=Path,
        default=Path(__file__).resolve().parent / "commercial-rti",
    )
    parser.add_argument(
    ...
    report = {
        "reference_rti": inspect(args.reference_rti, "java"),
        "gorti": inspect(args.gorti, "python"),
    }
    report["passed"] = report["reference_rti"]["passed"] and report["gorti"]["passed"]
