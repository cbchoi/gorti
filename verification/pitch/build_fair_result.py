"""Build the shared fair-comparison result from one verified Pitch run."""
...
def _attestation(evidence_path: Path, benchmark: dict[str, Any]) -> dict[str, Any]:
    ...
    if evidence.get("schema") != "gorti.pitch/run-evidence-v1":
        raise ValueError("Pitch launch evidence has an unknown schema")
    if evidence.get("status") != "attested":
        reason = evidence.get("reason") or "required launch evidence is absent"
        raise ValueError(f"Pitch launch is unattested: {reason}")
    recorded_hash = benchmark["metadata"]["provenance"].get("run_evidence_sha256")
    if recorded_hash != _sha256_file(evidence_path):
        raise ValueError("Pitch run-evidence SHA-256 differs from the benchmark attestation")

    runtime = evidence.get("runtime_artifacts")
    if not isinstance(runtime, dict):
        raise ValueError("Pitch runtime artifact evidence is absent")
    crc_jar = _artifact(runtime.get("crc_jar"), "Pitch CRC JAR")
    api_jar = _artifact(runtime.get("pitch_api_jar"), "Pitch API JAR")
    verifier_jar = _artifact(runtime.get("verifier_jar"), "Pitch verifier JAR")
    ...
    server = _process(
        server_value,
        "Pitch CRC",
        required_arguments=("-jar", crc_jar["path"], "-nogui")
        if server_lifecycle == "per_arm"
        else (),
        allowed_lifecycles=("per_arm", "persistent_session"),
    )
    if (
        server_lifecycle == "per_arm"
        and _argument_value(server, "-jar", "Pitch CRC") != crc_jar["path"]
    ):
        raise ValueError("Pitch CRC: -jar does not select the attested CRC JAR")
    clients = evidence.get("client_processes")
    if not isinstance(clients, dict):
        raise ValueError("Pitch verifier process evidence is absent")
    classpath = f"{verifier_jar['path']};{api_jar['path']}"
    publisher = _process(
        clients.get("publisher"),
        "Pitch publisher verifier",
        required_arguments=(
            "-cp",
            classpath,
    ...
    if _argument_value(publisher, "-cp", "Pitch publisher verifier") != classpath:
        raise ValueError("Pitch publisher verifier: -cp differs from attested JARs")
    if _argument_value(subscriber, "-cp", "Pitch subscriber verifier") != classpath:
        raise ValueError("Pitch subscriber verifier: -cp differs from attested JARs")
    if _argument_value(publisher, "--role", "Pitch publisher verifier") != "publisher":
        raise ValueError("Pitch publisher verifier: role argument is wrong")
    if _argument_value(subscriber, "--role", "Pitch subscriber verifier") != "subscriber":
        raise ValueError("Pitch subscriber verifier: role argument is wrong")
    if publisher["executable"].casefold() != subscriber["executable"].casefold():
        raise ValueError("Pitch verifier roles used different Java executables")
    ...
    if not isinstance(server_logs, dict) or not isinstance(client_logs, dict):
        raise ValueError("Pitch process log evidence is absent")
    for stream in ("stdout", "stderr"):
        _artifact(server_logs.get(stream), f"Pitch CRC {stream}")
        for role in ("publisher", "subscriber"):
            role_logs = client_logs.get(role)
            if not isinstance(role_logs, dict):
                raise ValueError(f"Pitch {role} log evidence is absent")
            _artifact(role_logs.get(stream), f"Pitch {role} {stream}")
    return evidence
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if _canonical(metadata["workload"]) != _canonical(workload):
        raise ValueError("Pitch benchmark workload differs from the orchestrator workload")

    repo = Path(__file__).resolve().parents[2]
    sys.path.insert(0, str(repo))
    from verification.common.project_semantics import project

    rows = project(_read_ndjson(args.canonical), "pitch", str(workload["seed"]), workload["count"])
    ...
    for sample in benchmark["samples"]:
        ...
        if operation not in OPERATIONS:
            raise ValueError(f"unsupported Pitch sample operation: {operation}")
    ...
    if observed != expected:
        raise ValueError(f"Pitch sample counts differ: {observed} != {expected}")
    ...
    if evidence is None:
        ...
        notes = "Pitch CRC and verifier process identities and captured logs are attested."
    return {
