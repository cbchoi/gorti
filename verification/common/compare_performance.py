...
def _reference_rti_samples(records: list[dict[str, Any]], metric: str) -> list[float]:
    return [
...
def _row(name: str, unit: str, reference_rti: float | None, gorti: float | None) -> dict[str, Any]:
    ratio = None if reference_rti in {None, 0.0} or gorti is None else gorti / reference_rti
    return {
        "metric": name,
        "unit": unit,
        "reference_rti": reference_rti,
        "gorti": gorti,
        "gorti_over_reference_rti": ratio,
    }


def compare(reference_rti: list[dict[str, Any]], gorti: list[dict[str, Any]]) -> dict[str, Any]:
    rows = []
    mappings = (
        (
    ...
    for name, reference_rti_name, gorti_name in mappings:
        reference_rti_values = _reference_rti_samples(reference_rti, reference_rti_name)
    ...
    rows.append(
        _row(
            ...
            _gorti_value(reference_rti, "sustained_throughput"),
...
def main() -> int:
    ...
    parser.add_argument("reference_rti", type=Path)
    parser.add_argument("gorti", type=Path)
    parser.add_argument("--report", type=Path, required=True)
    args = parser.parse_args()
    report = compare(_read(args.reference_rti), _read(args.gorti))
