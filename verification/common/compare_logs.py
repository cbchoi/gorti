...
def compare_records(
    pitch: list[dict[str, Any]], gorti: list[dict[str, Any]]
) -> dict[str, Any]:
    pitch_semantics = canonical_semantics(pitch)
    gorti_semantics = canonical_semantics(gorti)
    mismatches = []
    count = max(len(pitch_semantics), len(gorti_semantics))
    for index in range(count):
        left = pitch_semantics[index] if index < len(pitch_semantics) else None
        right = gorti_semantics[index] if index < len(gorti_semantics) else None
        if left != right:
            mismatches.append({"index": index, "pitch": left, "gorti": right})

    covered = {
        record["service"]
        for record in pitch_semantics + gorti_semantics
        if record.get("service") in SERVICES
    }
    missing_services = sorted(SERVICES - covered)

    pitch_metrics = _metrics(pitch)
    gorti_metrics = _metrics(gorti)
    metric_rows = []
    for key in sorted(set(pitch_metrics) | set(gorti_metrics)):
        left = pitch_metrics.get(key)
...
def compare_files(pitch_path: str | Path, gorti_path: str | Path) -> dict[str, Any]:
    return compare_records(read_ndjson(pitch_path), read_ndjson(gorti_path))


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("pitch_log", type=Path)
    parser.add_argument("gorti_log", type=Path)
    parser.add_argument("--report", type=Path)
    args = parser.parse_args()
    report = compare_files(args.pitch_log, args.gorti_log)
