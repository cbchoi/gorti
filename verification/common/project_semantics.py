...
def _validate_reference_rti(
    ...
    for actor in ("publisher", "subscriber"):
        ...
        _require(bool(passed), f"reference_rti {actor} did not reflect PASS")

    for event in ("object_published", "interaction_published"):
        _require(bool(_select(records, event, "publisher")), f"reference_rti missing {event}")
    for event in ("object_subscribed", "interaction_subscribed"):
        _require(bool(_select(records, event, "subscriber")), f"reference_rti missing {event}")
    _require(bool(_select(records, "object_name_reserved", "publisher")), "reference_rti name not reserved")
    _require(
        bool(_select(records, "object_registered", "publisher")),
        "reference_rti object not registered",
    )
    _require(
        bool(_select(records, "object_discovered", "subscriber")),
        "reference_rti object not discovered",
    )
    _require(bool(_select(records, "object_deleted", "publisher")), "reference_rti object not deleted")
    _require(bool(_select(records, "object_removed", "subscriber")), "reference_rti object not removed")
    ...
    for label in ("VERIFY_READY", "VERIFY_DONE"):
        for actor in ("publisher", "subscriber"):
            ...
            _require(bool(synchronized), f"reference_rti {actor} missing synchronized {label}")
    ...
    for actor in ("publisher", "subscriber"):
        ...
        _require(grants == expected_times, f"reference_rti {actor} grant set differs: {grants}")
    ...
    for index in range(count):
        ...
        for side in (sent_attributes[index], got_attributes[index]):
            _require(side["payload"] == expected_attribute, f"reference_rti attribute {index} differs")
            _require(int(side["logical_time"]) == index + 1, "reference_rti attribute time differs")
        for side in (sent_interactions[index], got_interactions[index]):
            _require(side["payload"] == expected_interaction, f"reference_rti interaction {index} differs")
            _require(int(side["logical_time"]) == index + 1, "reference_rti interaction time differs")
...
def project(
    ...
    if implementation == "reference_rti":
        attributes, interactions = _validate_reference_rti(records, seed, count)
    else:
...
def main() -> int:
    ...
    parser.add_argument("--implementation", choices=("reference_rti", "gorti"), required=True)
