...
func TestManager_RoutingSnapshotsInvalidateWithoutMutatingReaders(t *testing.T) {
	mgr := New()
	ctx := context.Background()
	if err := mgr.SubscribeObjectClassAttributes(ctx, "fed", 2, 7, []core.AttributeHandle{1}); err != nil {
		t.Fatal(err)
	}
	if err := mgr.SubscribeInteractionClass(ctx, "fed", 2, 11); err != nil {
		t.Fatal(err)
	}

	firstObject := mgr.ObjectSubscribersSnapshot(ctx, "fed", 7, []core.AttributeHandle{1})
	firstInteraction := mgr.InteractionSubscribersSnapshot(ctx, "fed", 11)
	if !reflect.DeepEqual(firstObject, []core.FederateHandle{2}) ||
		!reflect.DeepEqual(firstInteraction, []core.FederateHandle{2}) {
		t.Fatalf("initial snapshots = %v / %v", firstObject, firstInteraction)
	}

	if err := mgr.SubscribeObjectClassAttributes(ctx, "fed", 3, 7, []core.AttributeHandle{1}); err != nil {
		t.Fatal(err)
	}
	if err := mgr.SubscribeInteractionClass(ctx, "fed", 3, 11); err != nil {
		t.Fatal(err)
	}
	secondObject := mgr.ObjectSubscribersSnapshot(ctx, "fed", 7, []core.AttributeHandle{1})
	secondInteraction := mgr.InteractionSubscribersSnapshot(ctx, "fed", 11)
	if !reflect.DeepEqual(secondObject, []core.FederateHandle{2, 3}) ||
		!reflect.DeepEqual(secondInteraction, []core.FederateHandle{2, 3}) {
		t.Fatalf("expanded snapshots = %v / %v", secondObject, secondInteraction)
	}
	if !reflect.DeepEqual(firstObject, []core.FederateHandle{2}) ||
		!reflect.DeepEqual(firstInteraction, []core.FederateHandle{2}) {
		t.Fatalf("previous snapshots mutated = %v / %v", firstObject, firstInteraction)
	}

	mgr.OnFederateResign("fed", 2)
	if got := mgr.ObjectSubscribersSnapshot(ctx, "fed", 7, []core.AttributeHandle{1}); !reflect.DeepEqual(got, []core.FederateHandle{3}) {
		t.Fatalf("object snapshot after resign = %v", got)
	}
	if got := mgr.InteractionSubscribersSnapshot(ctx, "fed", 11); !reflect.DeepEqual(got, []core.FederateHandle{3}) {
		t.Fatalf("interaction snapshot after resign = %v", got)
	}

	mgr.OnFederationDestroyed("fed")
	if got := mgr.ObjectSubscribersSnapshot(ctx, "fed", 7, []core.AttributeHandle{1}); len(got) != 0 {
		t.Fatalf("destroyed object snapshot = %v", got)
	}
	if got := mgr.InteractionSubscribersSnapshot(ctx, "fed", 11); len(got) != 0 {
		t.Fatalf("destroyed interaction snapshot = %v", got)
	}
}

func TestManager_PublishesObjectAttributeTracksLifecycle(t *testing.T) {
	mgr := New()
	ctx := context.Background()
	if mgr.PublishesObjectAttribute(ctx, "fed", 7, 1, 2) {
		t.Fatal("unknown publication reported present")
	}
	if err := mgr.PublishObjectClassAttributes(ctx, "fed", 2, 7, []core.AttributeHandle{1}); err != nil {
		t.Fatal(err)
	}
	if !mgr.PublishesObjectAttribute(ctx, "fed", 7, 1, 2) {
		t.Fatal("published attribute not found")
	}
	if err := mgr.UnpublishObjectClassAttributes(ctx, "fed", 2, 7, []core.AttributeHandle{1}); err != nil {
		t.Fatal(err)
	}
	if mgr.PublishesObjectAttribute(ctx, "fed", 7, 1, 2) {
		t.Fatal("unpublished attribute remained present")
	}
	if err := mgr.PublishObjectClassAttributes(ctx, "fed", 2, 7, []core.AttributeHandle{1}); err != nil {
		t.Fatal(err)
	}
	mgr.OnFederateResign("fed", 2)
	if mgr.PublishesObjectAttribute(ctx, "fed", 7, 1, 2) {
		t.Fatal("resigned publisher remained present")
	}
}

func BenchmarkManagerRoutingSnapshots(b *testing.B) {
	mgr := New()
	ctx := context.Background()
	_ = mgr.SubscribeObjectClassAttributes(ctx, "fed", 2, 7, []core.AttributeHandle{1})
	_ = mgr.SubscribeInteractionClass(ctx, "fed", 2, 11)
	attributes := []core.AttributeHandle{1}
	_ = mgr.ObjectSubscribersSnapshot(ctx, "fed", 7, attributes)
	_ = mgr.InteractionSubscribersSnapshot(ctx, "fed", 11)
	...
	for range b.N {
		_ = mgr.ObjectSubscribersSnapshot(ctx, "fed", 7, attributes)
		_ = mgr.InteractionSubscribersSnapshot(ctx, "fed", 11)
		if !mgr.PublishesObjectAttribute(ctx, "fed", 7, 1, 99) {
			// Keep the lookup result observable without adding benchmark work.
		}
	}
}
