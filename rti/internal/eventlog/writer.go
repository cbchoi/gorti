...
var ErrNotImplemented = errors.New("eventlog: not implemented ( M2 deliverable)")

// errWriterClosed is returned by Append/Sync after Close.
var errWriterClosed = errors.New("eventlog: writer is closed")

// Keep the common small event frame on its federation writer. Larger payloads
// remain supported but are released after the synchronous sink write so one
// outlier cannot permanently raise the writer's retained memory.
const maxRetainedFrameCapacity = 64 << 10
...
type WriterOptions struct {
	...
	// AllowPartialProto skips Protobuf required-field initialization checks.
	// Encoding, UTF-8 checks, sequence assignment, and sink writes are unchanged.
...
type Writer struct {
	...
	mu           sync.Mutex // guards nextSeq, frameScratch, sink writes, closed
	nextSeq      uint64
	frameScratch []byte
	closed       bool
}
...
func (w *Writer) Append(ctx context.Context, fed core.FederationName, evt core.EventRecord) error {
	...
	frame := w.frameScratch
	if cap(frame) < 4 {
		frame = make([]byte, 4, 256)
	} else {
		frame = frame[:4]
	}
	frame, err := (proto.MarshalOptions{
		AllowPartial: w.opts.AllowPartialProto,
	}).MarshalAppend(frame, msg)
	if err != nil {
		w.nextSeq--
	...
	if cap(frame) <= maxRetainedFrameCapacity {
		w.frameScratch = frame[:0]
	} else {
		w.frameScratch = nil
	}
	return nil
