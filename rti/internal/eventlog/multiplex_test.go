...
func TestMultiplexWriter_ForwardsAllowPartialProto(t *testing.T) {
	var got bool
	mux, err := NewMultiplexWriter(MultiplexOptions{
		Clock:             core.NewFakeClock(time.Unix(0, 0)),
		Mode:              core.ModeVerbose,
		AllowPartialProto: true,
		Factory: func(opts WriterOptions) (*Writer, error) {
			got = opts.AllowPartialProto
	...
	if err := mux.Append(
		context.Background(),
		"alpha",
		&directProtoEvent{pb: &rtiv1.Event{WallNs: 99}},
	); err != nil {
		t.Fatalf("Append: %v", err)
	}
	if !got {
		t.Fatal("WriterOptions.AllowPartialProto = false, want true")
	}
}
...
func BenchmarkMultiplexWriter_AppendDirectSeqAdapterReuse(b *testing.B) {
	benchmarkMultiplexWriterAppendDirectSeqAdapterReuse(b, false)
}

func BenchmarkMultiplexWriter_AppendDirectSeqAdapterReuseValidationOff(b *testing.B) {
	benchmarkMultiplexWriterAppendDirectSeqAdapterReuse(b, true)
}

func benchmarkMultiplexWriterAppendDirectSeqAdapterReuse(b *testing.B, allowPartialProto bool) {
	mux, err := NewMultiplexWriter(MultiplexOptions{
		Clock:             core.NewFakeClock(time.Unix(0, 0)),
		Mode:              core.ModeVerbose,
	...
	b.Cleanup(func() { _ = mux.Close() })
	ctx := context.Background()
	evt := &directProtoEvent{pb: &rtiv1.Event{WallNs: 1}}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		evt.pb.WallNs = uint64(i)
		if err := mux.Append(ctx, "bench", evt); err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkMultiplexWriter_AppendInteractionValidation(b *testing.B) {
	for _, tc := range []struct {
		name              string
		allowPartialProto bool
	}{
		{name: "on"},
		{name: "off", allowPartialProto: true},
	} {
		b.Run(tc.name, func(b *testing.B) {
			mux, err := NewMultiplexWriter(MultiplexOptions{
				Clock:             core.NewFakeClock(time.Unix(0, 0)),
				Mode:              core.ModeVerbose,
				AllowPartialProto: tc.allowPartialProto,
				Factory: func(opts WriterOptions) (*Writer, error) {
					opts.Sink = io.Discard
					return NewWriter(opts)
				},
				...
				Body: &rtiv1.Event_InterSent{
					InterSent: &rtiv1.InteractionSent{
						...
						Parameters: map[uint64][]byte{
							11: bytes.Repeat([]byte{0x5a}, 256),
							12: bytes.Repeat([]byte{0xa5}, 256),
						},
