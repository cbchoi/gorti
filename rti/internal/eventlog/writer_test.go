...
func TestWriter_AllowPartialProtoPreservesValidEventBytes(t *testing.T) {
	write := func(allowPartialProto bool) []byte {
		t.Helper()
		...
		if err := w.Append(context.Background(), "demo", evt); err != nil {
	...
	strict := write(false)
	partial := write(true)
	if !bytes.Equal(strict, partial) {
		t.Fatalf("event-log bytes differ:\nstrict = %x\npartial = %x", strict, partial)
	}
}
...
func TestWriter_AppendReusesSmallFrameAndReleasesLargeFrame(t *testing.T) {
	w, err := NewWriter(WriterOptions{
		Sink: io.Discard, Federation: "demo", Mode: core.ModeVerbose,
		Clock: core.NewFakeClock(time.Unix(0, 0)),
	})
	if err != nil {
		t.Fatalf("NewWriter: %v", err)
	...
	small := &directProtoEvent{pb: &rtiv1.Event{WallNs: 1}}
	if err := w.Append(context.Background(), "demo", small); err != nil {
		t.Fatalf("Append small: %v", err)
	}
	smallCapacity := cap(w.frameScratch)
	if smallCapacity == 0 || smallCapacity > maxRetainedFrameCapacity {
		t.Fatalf("small frame retained capacity = %d", smallCapacity)
	}
	if err := w.Append(context.Background(), "demo", small); err != nil {
		t.Fatalf("Append second small: %v", err)
	}
	if cap(w.frameScratch) != smallCapacity {
		t.Fatalf("second small frame capacity = %d, want %d", cap(w.frameScratch), smallCapacity)
	}

	large := &directProtoEvent{pb: &rtiv1.Event{
		Body: &rtiv1.Event_AttrUpdated{AttrUpdated: &rtiv1.AttributeUpdated{
			Attributes: map[uint64][]byte{1: make([]byte, maxRetainedFrameCapacity+1)},
		}},
	}}
	if err := w.Append(context.Background(), "demo", large); err != nil {
		t.Fatalf("Append large: %v", err)
	}
	if w.frameScratch != nil {
		t.Fatalf("large frame retained capacity = %d, want 0", cap(w.frameScratch))
	}
}
