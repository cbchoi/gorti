...
// ErrNotImplemented is returned by methods that are still stubs.
// Spec tests in tests/spec/M2/ will fail with this error initially (RED), then
// turn green as functionality is added.
var ErrNotImplemented = errors.New("federation: not implemented ( M2 deliverable)")
...
type federationState struct {
	...
	// reference_rti, MAK) and the behavior an online algorithm can guarantee.
	//
...
//
// This matches industry RTI behavior (Portico, reference_rti, MAK) and is the
...
// AcquireMemberLease is the value-type variant of AcquireMember (W2):
// identical validation and fencing, but the shared lease is returned as
// a core.MemberLease value so hot-path callers avoid the bound release
// closure heap allocation.
//
// The two-phase order below — fs.operations.RLock FIRST, then m.mu
// revalidation — is replicated verbatim from AcquireMember and is
// deliberate. Do NOT fold the double m.mu.RLock into one critical
// section: holding m.mu while blocking on fs.operations (or vice versa
// in one section) is a deadlock hazard against destroy/resign, which
// take fs.operations.Lock before m.mu.Lock.
func (m *Manager) AcquireMemberLease(fed core.FederationName, h core.FederateHandle) (core.MemberLease, error) {
	if h == core.InvalidFederateHandle {
		return core.MemberLease{}, core.ErrFederateNotJoined
	}
	m.mu.RLock()
	fs, ok := m.federations[fed]
	m.mu.RUnlock()
	if !ok {
		return core.MemberLease{}, core.ErrFederationNotFound
	}
	fs.operations.RLock()
	m.mu.RLock()
	current, exists := m.federations[fed]
	...
	return core.NewMemberLease(&fs.operations), nil
}
