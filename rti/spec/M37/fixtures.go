// Package m37spec — M37 spec tests for the additive
// FederateEvent proto-slot batch (docs/COMMERCIAL_RTI_PARITY.md "M37 backlog").
//
...
func (o *fakeOutbox) Reserve(_ context.Context, fed core.FederationName, deliveries []core.OutboxDelivery) (core.OutboxReservation, error) {
	return &fakeOutboxReservation{
		outbox: o, fed: fed, deliveries: append([]core.OutboxDelivery(nil), deliveries...),
	}, nil
}

type fakeOutboxReservation struct {
	mu         sync.Mutex
	outbox     *fakeOutbox
...
func (r *fakeOutboxReservation) Commit() error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.done {
		return nil
	...
	for _, delivery := range r.deliveries {
		r.outbox.sent = append(r.outbox.sent, sentRecord{
			Federation: r.fed,
...
func (r *fakeOutboxReservation) Release() {
	r.mu.Lock()
	r.done = true
	r.mu.Unlock()
}
...
var _ core.ReservableOutbox = (*fakeOutbox)(nil)
