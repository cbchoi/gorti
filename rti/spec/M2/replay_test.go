...
//
// Until eventlog.Replayer and the supporting components land, this test
// uses t.Skip with a clear reason and becomes active when they are ready.
//
...
func TestSpec_M2_Replay_ByteIdentical(t *testing.T) {
	...
	rep, err := eventlog.NewReplayer(eventlog.ReplayerOptions{
		Source:        r,
		Federation:    nil, // wired by M2 integration
		Objects:       nil,
		CapturingSink: w,
	})
	if err != nil {
...
// buildExampleLog produces a small, deterministic M2 log
...
func TestSpec_M2_Replay_DivergenceIsFatal(t *testing.T) {
	t.Skip("scaffolded; activates with the divergence-injection harness once Replayer lands")
}
