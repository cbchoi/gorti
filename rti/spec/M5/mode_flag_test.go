...
func TestSpec_M5_ModeFlag_VerboseDefault(t *testing.T) {
	...
	if got.Mode == core.ModeUnspecified {
		...
		// TASK-076 wires the default-to-Verbose normalization
		// — at the federation manager OR the gRPC handler. Whichever
		// path the implementation chooses, this test then asserts ModeVerbose
		// flows through. A future integration step converts this skip to a hard
