"""M17.7 — cross-language reference_rti smoke: C++ publisher ↔ Python subscriber.

Spawns:
  1. rtid as a subprocess on a free port
  2. examples/cpp-ieee1516e-smoke/cpp_ieee1516e_publisher (built via
...
federate using only IEEE 1516 service-style C++ ambassador methods can fan out
its events to a Python subscriber using only IEEE 1516 service-style Python
...
CPP_PUBLISHER = (
    REPO_ROOT / "cppsdk" / "build" / "examples" / "cpp-ieee1516e-smoke"
    / "cpp_ieee1516e_publisher"
)
CPP_FOM = REPO_ROOT / "examples" / "cpp-ieee1516e-smoke" / "federation.fom.xml"
FEDERATION = "cpp-ieee1516e-smoke"
...
def test_spec_m17_7_cpp_publisher_python_subscriber() -> None:
    ...
    if not CPP_PUBLISHER.exists():
        pytest.skip(
            f"cpp_ieee1516e_publisher not built at {CPP_PUBLISHER}; "
