"""M27 Phase D — cross-federate IEEE 1516 ambassador-surface smoke.

Broadens the M26 Phase G single-publisher smoke with a SUBSCRIBER
federate that uses ONLY IEEE 1516 service-style ambassador methods (override
callback slots + evokeMultipleCallbacks loop). Verifies that
discoverObjectInstance / reflectAttributeValues / receiveInteraction
fire end-to-end through the IEEE 1516 service surface — no reach-around into
events.async iteration.

This is the final verification that gorti's Layer 2 ambassador is
usable as a IEEE 1516 service-style federate harness, not just a publisher
proof-of-concept.
"""
...
def test_spec_m27d_ieee1516e_ambassador_cross_federate() -> None:
    """Publisher + subscriber both using ONLY IEEE 1516 service-style methods.

    The subscriber observes Discover + Reflect + Receive callbacks
    fired through the override slots — proving the Layer 2
    ambassador is sufficient for a port from reference_rti / Portico / MAK
    ...
    try:
        ...
        sys.path.insert(0, str(REPO_ROOT / "examples" / "ieee1516e-ambassador-smoke"))
        try:
