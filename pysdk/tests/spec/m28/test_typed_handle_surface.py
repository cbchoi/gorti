"""M28 W4 — reference_rti typed-handle public-namespace lockfile.
...
``rti1516e`` namespace and behaves correctly:
  ...
  - ``getAttributeHandleSetFactory().create()`` returns
    ``AttributeHandleSet``.
  - IEEE 1516 service-style imports + construction round-trip works.
...
@pytest.mark.spec
def test_spec_m28_ieee1516e_import_path_roundtrip() -> None:
    """IEEE 1516 service-style code path works: import the typed handle + typed
...
def test_spec_m28_typed_handle_int_equality_documented_concession() -> None:
    ...
    deliberate dual-accept concession documented in COMMERCIAL_RTI_PARITY.md.
    ...
    # because both are int(5). Documented concession per COMMERCIAL_RTI_PARITY.md.
