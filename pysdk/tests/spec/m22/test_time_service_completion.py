...
def test_ac_3_3_workaround_removed() -> None:
    ...
    Source-level check; the literal string is also pinned by
    scripts/check-milestones.sh's M22 probe.
    """
    ...
    assert "for attempt in range(8)" not in text, (
        "M21-era retry-with-backoff loop still present; W3 workaround removal incomplete"
    )
