...
are currently exposed through query methods rather than stream events.

The production server's savepoint manager runs in *dynamic* mode without
a MembersResolver. In this mode, the first federate to call
:meth:`federate_save_complete` adds itself to the required set, and
the save closes out as ``SAVED`` immediately. This supports
single-federate save/restore round trips; multi-federate aggregation
requires server-side MembersResolver wiring.
"""
